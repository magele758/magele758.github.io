...
The plan is ready for your review. Two minimal changes:

1. **Fix JSON-RPC ID collision** — replace `Date.now()` with an incrementing counter in `mcp-jsonrpc.ts`
2. **Add tests** — new `mcp-jsonrpc.test.js` covering `parseMcpUrls`, `mcpListTools`, and `mcpCallTool` using Node.js built-in HTTP mock servers
...
ok 5 - HybridModelRouterAdapter last_user scope ignores images only in older turns
  ...
  type: 'test'
  ...
# (node:66951) ExperimentalWarning: SQLite is an experimental feature and might change at any time
